  ...
  if (!drama || !activeEpisodeDoc) {
    return (
      <div className="h-screen w-full bg-transparent flex flex-col items-center justify-center gap-3">
        ...
        <Link href={`/drama/${slug}`} className="px-4 py-2 bg-brand-primary rounded-xl text-xs font-bold text-white">Back to Details</Link>
      </div>
    );
  }
                ...
                {sortedSubtitles.map((sub) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      const customFileName = `${cleanDramaTitle}_S${formattedSeason}_E${formattedEpisode}_${subLang}.${sub.format || 'srt'}`;
