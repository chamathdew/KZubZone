...
export default function GlassCard({ item, type }) {
  ...
  return (
    <Link to={detailsUrl} className="block relative group w-full min-w-0">
      <motion.div
        whileHover={{ y: -6, scale: 1.02 }}
        transition={{ duration: 0.4, ease: [0.25, 0.8, 0.25, 1] }}
        className="relative flex aspect-[2/3] w-full min-w-0 flex-col overflow-hidden rounded-lg bg-luxury-900 border border-white/5 shadow-md shadow-black/40 group-hover:shadow-brand-primary/20 group-hover:border-brand-primary/30 transition-shadow duration-300"
      >
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between gap-2 z-20">
          ...
          <div className="min-w-0">
            {mediaType === 'drama' && hasSubtitles ? (
              <span className={`inline-flex items-center justify-center rounded-md border px-2 py-0.5 font-sans backdrop-blur-md text-[10px] font-black uppercase tracking-wider ${
          ...
          <div className={`flex flex-shrink-0 items-center gap-1 rounded-md border px-2 py-0.5 font-sans backdrop-blur-md ${getRatingBadgeClass(rating)}`}>
            <Star className="w-2.5 h-2.5 fill-current" />
            <span className="text-[10px] font-black">{rating > 0 ? `IMDb ${rating.toFixed(1)}` : 'NR'}</span>
          </div>
        </div>

        {/* Drama / Movie label — bottom-right floating pill, fades out on hover */}
        <div className="absolute bottom-14 right-3 z-20 transition-opacity duration-200 group-hover:opacity-0">
        ...
        {/* Permanent bottom dark gradient overlay for text readability (both default and hover states) */}
        <div className="absolute inset-x-0 bottom-0 h-[60%] bg-gradient-to-t from-black/95 via-black/60 to-transparent z-10 pointer-events-none" />

        {/* Default Title & Meta shown before hover (fades out on hover) */}
        <div className="absolute inset-x-0 bottom-0 p-3 z-20 flex flex-col gap-1 transition-opacity duration-200 group-hover:opacity-0 text-left">
