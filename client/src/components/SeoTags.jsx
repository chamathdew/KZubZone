...
export default function SeoTags({ 
  ...
  useEffect(() => {
    ...
    let schemaScript = head.querySelector('script[id="kdramaverse-jsonld-schema"]');
    if (schemaMarkup) {
      if (!schemaScript) {
        schemaScript = document.createElement('script');
        schemaScript.setAttribute('type', 'application/ld+json');
        schemaScript.setAttribute('id', 'kdramaverse-jsonld-schema');
    ...
    return () => {
      const cleanupScript = head.querySelector('script[id="kdramaverse-jsonld-schema"]');
      if (cleanupScript) {
