    ...
    return (
      <div className="min-h-screen bg-transparent flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <Link to="/articles" className="mt-4 inline-flex px-4 py-2 rounded-xl bg-brand-primary text-white text-xs font-bold">
            Back to Articles
          </Link>
        </div>
      </div>
      ...
      <article>
        <section className="relative min-h-[58vh] overflow-hidden flex items-end border-b border-white/5">
          ...
          <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 w-full pb-12 pt-28 text-left">
            <Link to="/articles" className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-slate-300 hover:text-white mb-6">
          ...
          {related.length > 0 && (
            <div className="mt-12 border-t border-white/5 pt-8">
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                {related.map((item) => (
                  <Link key={item._id} to={`/articles/${item.slug}`} className="rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden hover:border-brand-primary/40 transition">
